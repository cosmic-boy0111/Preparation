...
nodeInfo findLargestBST(node* root){
    ...
    rootInfo.max = max(root->val, right.max);
    rootInfo.sum = root->val + left.sum + right.sum;
    rootInfo.maxSum = rootInfo.isBST ? max(left.maxSum, max(right.maxSum, rootInfo.sum)) : max(left.maxSum, right.maxSum);
    return rootInfo;
}
